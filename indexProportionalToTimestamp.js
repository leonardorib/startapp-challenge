...
for (let i = 0; i < newList.length - 1; i++) {
  if (newList[i].value == null) {
    ...
    // [Mudança] Calcula a taxa de variação do valor ao invés do incremento
    const dValue_dT =
      (newList[i + nullCount].value - newList[i - 1].value) /
      (newList[i + nullCount].timestamp - newList[i - 1].timestamp);
    ...
    for (let x = i; inNullInterval == true; x++) {
      if (newList[x].value !== null) {
        ...
        // [Mudança] Adiciona (dValor/dT * deltaT) ao invés de um incremento fixo
        newList[x].value =
          newList[x - 1].value +
          dValue_dT * (newList[x].timestamp - newList[x - 1].timestamp);
      }
    }
  }
}
